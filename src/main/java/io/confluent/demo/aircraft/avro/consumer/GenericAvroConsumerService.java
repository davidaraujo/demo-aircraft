        ...
        try {
            while (true) {
                ...
                for (ConsumerRecord<String, GenericRecord> record : records) {
                    PrettyPrint.consumerRecord(groupId, ((clientId == null)) ? "Unidentified": clientId, topicName, record.partition(), record.offset(), record.key().toString(), record.value().toString());
                }
            }
        } finally {
            consumer.close();
