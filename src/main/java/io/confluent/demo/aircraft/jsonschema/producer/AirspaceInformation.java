...
/**
 * <h1>Produce JSON flight messages to Kafka and use Schema Registry for JSON Schema format validation</h1>
 ...
 * mvn exec:java -Dexec.mainClass="io.confluent.demo.flights.jsonschema.producer.AirspaceInformation" -Dexec.args="./src/main/resources ccloud_prod.properties flights opensky.properties OpenSKY-App"
 * java -cp ~/.m2/repository/org/apache/kafka/kafka-clients/2.5.0/kafka-clients-2.5.0.jar:~/.m2/repository/org/slf4j/slf4j-api/1.7.5/slf4j-api-1.7.5.jar:target/demo-flights-1.0-SNAPSHOT.jar io.confluent.demo.flights.jsonschema.ProducerApp ./src/main/resources ccloud_prod.properties flights opensky.properties OpenSKY-App
 * </pre>
 *
 * @author David Araujo
 * @version 1.1
...
public class AirspaceInformation implements Runnable {
    ...
    private String openSkyPropsFile;
    ...
    public void publishAirspaceInformation()
        ...
        producerProps.setProperty("value.serializer", "io.confluent.kafka.serializers.json.KafkaJsonSchemaSerializer");

        // Set validation for JSONSchema
        producerProps.setProperty("json.fail.invalid.schema", "true");
        ...
        // Authentication on the OpenSKY network
        ...
        while (true) {
            ...
            FlightStateSchema1 flight;

            // Iterate over the collection of flights information and produce messages to Kafka
            while (it.hasNext()) {
                try {
                        ...
                        public void onCompletion(RecordMetadata m, Exception e) {
                            if (e != null) {
                                ...
                                System.out.print(ColouredSystemOutPrintln.ANSI_BLACK + ColouredSystemOutPrintln.ANSI_BG_GREEN +
                                        "Client " + clientId + " producing message to topic " + m.topic() + " partition " + m.partition() + " [" + m.partition() + " ] @ offset " + m.offset() + ColouredSystemOutPrintln.ANSI_RESET);
                                System.out.print("\n" + ColouredSystemOutPrintln.ANSI_WHITE + ColouredSystemOutPrintln.ANSI_BG_BLUE);
                                System.out.println(new FlightPrettyPrint().printFlightV1((FlightStateSchema1) copyValue) + ColouredSystemOutPrintln.ANSI_RESET);
                            }
                        }
                    });
    ...
    public static void main(final String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        if (numberThreads == 0) {
            AirspaceInformation airspaceInformation = new AirspaceInformation(resourcesDir, propertiesFile, topicName, openSkyPropsFile, clientId);
            new Thread(airspaceInformation, clientId).start();
        }
        // run multiple producer threads
        else for (int i = 0; i < numberThreads; i++) {
            AirspaceInformation airspaceInformation = new AirspaceInformation(resourcesDir, propertiesFile, topicName, openSkyPropsFile, clientId + "-" + i);
            new Thread(airspaceInformation, clientId).start();
        }
    }
